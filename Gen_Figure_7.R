...
X1_demean <- X1 - final_centroids[final_cluster_k_3_neg, , drop = FALSE]
cov_sample <- coop::covar(X1_demean)
...
final_cell_types <- TS_bone_marrow$cell_ontology_class[which(
  (TS_bone_marrow$donor == "TSP14") &
    (TS_bone_marrow$cell_ontology_class %in%
      c("memory b cell", "monocyte", "nk cell", "macrophage"))
)]

table(final_cell_types, final_cluster_k_3_neg)
...
df_p0_summary <- df_p0_summary %>% drop_na()
# df_p0_summary$gene_names <- rep(names(X1), times = 6)
...
plot_fdr_null_num_rej_df <- plot_fdr_null_num_rej %>%
  mutate(difference = fdr_naive_rej - fdr_selective_rej) %>%
...
ggplot(
  plot_fdr_null_num_rej_by_pair_df %>%
    filter(fdr_cutoff <= 0.25),
  aes(x = fdr_cutoff, y = num_of_rejections, colour = method),
) +
  geom_line(size = 1.5) +
